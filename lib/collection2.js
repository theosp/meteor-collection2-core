...
Mongo.Collection.prototype.allowSchemaCustomFields = function c2AllowCustomFields() {
  // Custom fields are fields that aren't defined in the schema attached to the
  // collection.
  //
  // Once custom fields are allowed, they won't be filtered out from insert/update calls,
  // and the keyNotInSchema exception won't be raised.
  //
  // At the moment, once custom fields are enabled for a collection, the feautre cannot
  // be disabled.

  var self = this;

  // If a schema is already attached, re-attach it to enable custom fields in the SimpleSchema
  // level.
  self._c2 = self._c2 || {};
  if (self._c2._simpleSchema) {
    self.attachSchema(self._c2._simpleSchema, {replace: true});
  }
...
Mongo.Collection.prototype.attachSchema = function c2AttachSchema(ss, options) {
  ...
  var simple_schema_options = {}
  if (self.custom_fields_enabled) {
    simple_schema_options.allow_custom_fields = true;
  } else {
    simple_schema_options.allow_custom_fields = false;
  }

  // Allow passing just the schema object
  if (!(ss instanceof SimpleSchema)) {
    ss = new SimpleSchema(ss, simple_schema_options);
  }
  ...
  if (self._c2._simpleSchema && options.replace !== true) {
    if (ss.version >= 2) {
      var newSS = new SimpleSchema(self._c2._simpleSchema, simple_schema_options);
      newSS.extend(ss);
      ss = newSS;
    } else {
      ss = new SimpleSchema([self._c2._simpleSchema, ss], simple_schema_options);
    }
  }
    ...
    if (typeof selector === "object") {
      ...
      if (schemaIndex === -1) {
        ...
        obj._c2._simpleSchemas.push({
          schema: new SimpleSchema(ss, simple_schema_options),
          selector: selector,
        });
      } else {
        ...
        if (options.replace !== true) {
          ...
          if (obj._c2._simpleSchemas[schemaIndex].schema.version >= 2) {
            ...
            obj._c2._simpleSchemas[schemaIndex].schema = new SimpleSchema([obj._c2._simpleSchemas[schemaIndex].schema, ss], simple_schema_options);
          }
        } else {
